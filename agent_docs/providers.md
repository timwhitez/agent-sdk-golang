...
- OpenAI Chat and Responses (buffered, streaming and in-stream error events) set `ProviderError.Reason = context_overflow` only for the documented structured error code `context_length_exceeded`; the same status or message text without that code stays unclassified.
