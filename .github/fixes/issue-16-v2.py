...
'''
if text.count(old) != 1:
    raise SystemExit(f"planAdd content anchor count={text.count(old)}")
text = text.replace(old, new, 1)
...
'''
if text.count(old) != 1:
    raise SystemExit(f"planMove anchor count={text.count(old)}")
text = text.replace(old, new, 1)
...
'''
if text.count(old) != 1:
    raise SystemExit(f"planUpdate anchor count={text.count(old)}")
text = text.replace(old, new, 1)

old = '''func (p *patchPlanner) commit() error {
\tapplied := []string{}
\tfor i, a := range p.actions {
\t\tif err := p.commitAction(a); err != nil {
'''
new = '''func (p *patchPlanner) commit() error {
...
'''
if text.count(old) != 1:
    raise SystemExit(f"commit anchor count={text.count(old)}")
text = text.replace(old, new, 1)

start = text.index("// commitAction performs a single staged action")
end = text.index("\nvar beforePatchMoveNoReplace", start)
replacement = r'''func verifyPatchTargetUnchanged(expected map[string]writeTargetSnapshot, key, path, displayPath string) error {
...
func TestPatchPlannerRejectsUpdatedFileChangedAfterPlanning(t *testing.T) {
	...
	if err := planner.planUpdate("target.txt", []patchHunk{singleLinePatchHunk("old", "patched")}); err != nil { t.Fatal(err) }
	if err := os.WriteFile(target, []byte("concurrent writer\n"), 0o644); err != nil { t.Fatal(err) }
	if err := planner.commit(); !errors.Is(err, errStaleWriteTarget) { t.Fatalf("commit() error = %v", err) }
...
func TestPatchPlannerRejectsDeleteTargetReplacedAfterPlanning(t *testing.T) {
	...
	s, _ := New(root)
	planner := newPatchPlanner(s)
	if err := planner.planDelete("target.txt"); err != nil { t.Fatal(err) }
...
func TestPatchPlannerRejectsMoveSourceChangedAfterPlanning(t *testing.T) {
	...
	s, _ := New(root)
	planner := newPatchPlanner(s)
	if err := planner.planMove("source.txt", "destination.txt"); err != nil { t.Fatal(err) }
	...
	if _, err := os.Stat(destination); !os.IsNotExist(err) { t.Fatalf("destination unexpectedly created: %v", err) }
}
...
func TestPatchPlannerMultipleUpdatesDoNotConflictWithOwnWrites(t *testing.T) {
	...
	s, _ := New(root)
	planner := newPatchPlanner(s)
	if err := planner.planUpdate("target.txt", []patchHunk{singleLinePatchHunk("old", "first")}); err != nil { t.Fatal(err) }
	if err := planner.planUpdate("target.txt", []patchHunk{singleLinePatchHunk("first", "second")}); err != nil { t.Fatal(err) }
	if err := planner.commit(); err != nil { t.Fatalf("commit() error = %v", err) }
